...
class ReportRenderer:
    ...
    def markdown(self, facts: ReportFacts, summary: str) -> str:
        ...
        fraction = facts.changed_fraction if facts.changed_fraction is not None else "无法判断"
        denominator = (
            facts.change_area_denominator_km2
            if facts.change_area_denominator_km2 is not None
            else "无法判断"
        )
        return "\n".join(
            [
                ...
                f"- 变化面积：{change}",
                f"- 变化比例：{fraction}",
                f"- 比例分母：{denominator} km² "
                f"({facts.change_fraction_denominator or 'unavailable'})",
