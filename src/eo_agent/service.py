...
from eo_agent.scientific.engine import ScientificEngine
...
class TaskService:
    def __init__(
        ...
        *,
        ...
        scientific_llm: LLMClient | None = None,
        scientific_environment: EpisodeEnvironment | None = None,
    ) -> None:
    ...
    def run(self, request: TaskRequest) -> TaskRunResult:
        ...
        if (
            request.workflow_mode == WorkflowMode.LEGACY
            and request.scenario not in settings.scenarios
        ):
        ...
        if request.workflow_mode == WorkflowMode.SCIENTIFIC:
            return self._run_scientific(task_id, request, settings, artifacts)
        state: WorkflowState = {
            "request": request,
            "task_id": task_id,
    ...
    def _run_scientific(self, task_id, request, settings, artifacts) -> TaskRunResult:
        ...
        try:
            ...
            all_artifacts = [failure_ref]
            result = TaskRunResult(
                task_id=task_id,
                status=TaskStatus.FAILED,
                contains_mock=True,
                steps=["scientific: 任务失败，已保存实际错误"],
                report_html=None,
                report_json=None,
                error=error,
            )
