...
class TaskRequest(StrictModel):
    ...
    episode_id: str | None = Field(default=None, pattern=r"^[A-Za-z0-9_-]{1,64}$")

    @model_validator(mode="after")
    def validate_mode_fields(self) -> TaskRequest:
        if self.workflow_mode == WorkflowMode.SCIENTIFIC and not self.episode_id:
            raise ValueError("scientific 模式必须提供 episode_id")
        if self.workflow_mode == WorkflowMode.LEGACY and self.episode_id is not None:
            raise ValueError("legacy 模式不接受 episode_id")
        return self
...
class ChangeData(StrictModel):
    ...
    area_denominator_km2: float | None = Field(default=None, gt=0)
    area_denominator_kind: Literal["task_aoi_area", "readable_area"] | None = None
    can_determine: bool
...
class ReportFacts(StrictModel):
    ...
    change_area_km2: float | None
    changed_fraction: float | None
    change_area_denominator_km2: float | None = None
    change_fraction_denominator: Literal["task_aoi_area", "readable_area"] | None = None
