...
def test_scientific_api_is_explicit_and_reuses_task_endpoints(tmp_path) -> None:
    client = TestClient(create_app(tmp_path))
    response = client.post(
        "/api/tasks/run",
        json={
    ...
    assert task["workflow_mode"] == "scientific"
    report = client.get(f"/api/tasks/{result['task_id']}/report?format=json").json()
    assert report["workflow_mode"] == "scientific"
    assert report["events"][0]["decision"]["label"] == "transient_change"
