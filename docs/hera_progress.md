...
## 交互式影像准备 V1（独立增量，未进入 P2）

- 在同一 `eo_agent` 包与 FastAPI 中新增 `/imagery`，没有创建第二项目，也没有改变 legacy/scientific
  分发默认值。
- 完成 Mock 与 Earth Engine 统一 provider、AOI 内光学质量、SAR 候选、完整 LLM I/O 审计、
  SQLite/SSE、版本化审批、分块传输、GeoTIFF 校验和本地缩略图。
- 默认仍为离线 Mock；真实 DeepSeek/第二兼容模型、Earth Engine 连接及真实下载均未验证。
- 此增量只准备影像，不实现 P2 科学调查方法、去云推理或变化检测。
- 最终回归：`pytest -q` 为 60 passed，`ruff check .` 通过；legacy cloudy 与 scientific EP001
  均保持 `COMPLETED`。
- 浏览器完成任务 `IMG-7af2375ef2d04a58b7b6`：修改保存目录后计划从 v1 更新为 v2，用户确认后
  6/6 个模拟 GeoTIFF 通过打开、范围、波段、网格、有效像元和 SHA-256 校验；SQLite 保存
  129 条事件、3 次模型调用与审批记录，刷新页面未重复下载。
- Mock 任务产物位于 `outputs/browser_downloads/IMG-7af2375ef2d04a58b7b6/`；控制数据库位于
  `outputs/browser_check/imagery.sqlite3`。运行时产物保持在 git 忽略范围内。
